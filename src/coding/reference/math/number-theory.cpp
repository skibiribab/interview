// Number theory — modular exponentiation, factorials mod p, gcd, factor helpers
// When: combinatorics modulo prime, nCr mod p, small multiplicative tricks.
// Time: O(log MOD) modexp; O(√n) trial factor demos; varies by routine    Space: O(n) for factorial tables in demos
#include <algorithm>
#include <cassert>
#include <cmath>
#include <iostream>
...
// ≡ a^(-1) (mod MOD).
std::vector<int> build_inverse_factorial(const std::vector<int>& fact, const int MOD) {
...
int combination(const int set_size, const int subset_size, const std::vector<int>& fact, const std::vector<int>& ifact,
                const int MOD) {
    ...
    const int den = (1LL * ifact[set_size - subset_size] * ifact[subset_size]) % MOD;
...
std::vector<int> get_lowest_factors(const int MAX_SIZE) {
    ...
    for (int prime = 3; prime < lowest_factors.size(); prime += 2) {
        ...
        for (int multiple = prime; multiple < lowest_factors.size(); multiple += prime) {
            if (lowest_factors[multiple] != multiple) {
                continue; // Skip numbers that already have a lower prime factor.
            }
            lowest_factors[multiple] = prime; // Set the lowest prime factor for the multiple.
        }
    }
...
// array (LPF).
std::unordered_map<int, int> get_factors(const int N, const std::vector<int>& lowest_factors) {
    std::unordered_map<int, int> factors;
    // Continue dividing N by its lowest prime factor until N becomes 1.
    for (int i = N; i > 1; i /= lowest_factors[i]) {
        factors[lowest_factors[i]] += 1; // Increment the exponent of the prime factor.
    }
    return factors;
}
...
int arithmetic_progression_element(const int value, const int pos_start, const int pos_finish,
...
// the number of terms.
int arithmetic_progression_sum(const int value_start, const int value_finish, const int n) {
...
int geometric_progression_element(const int value, const int pos_start, const int pos_finish, const int common_ratio,
                                  const int MOD) {
    return (1LL * value * exponentiate(common_ratio, pos_finish - pos_start, MOD)) % MOD;
}

// Get the sum of a geometric progression (GP) given the initial value, common
// ratio, and number of terms.
int geometric_progression_sum(const int value, const int common_ratio, const int n, const int MOD) {
    if (common_ratio == 1)
        return value * n;
    // Use the formula: sum = a * (1 - r^n) / (1 - r)
    int num = (1LL * value * (1 - exponentiate(common_ratio, n, MOD) + MOD)) % MOD;
...
int mod_inverse(int a, const int MOD) {
    return exponentiate(a, MOD - 2, MOD);
}
