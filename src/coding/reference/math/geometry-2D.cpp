// Computational geometry — 2D points, lines, polygons, convex hull snippets
// When: intersections, areas, closest pairs (subset of routines in this file).
// Time: typical O(n)–O(n log n) per algorithm section    Space: O(n) for hull output etc.
#include <algorithm>
#include <cassert>
#include <cmath>
#include <iomanip>
...
bool _2D::is_polygon_convex(const vector<pv>& v) {
    ...
    for (int i = 0; i < n; ++i) {
        double cross_product = _2D::cross(v[(i + 1) % n] - v[i], v[(i + 2) % n] - v[i]);
...
double _2D::distance_line_line(const lrs& l1, const lrs& l2) {
    ...
    return fabs(_2D::cross(l2.start - l1.start, l1.direction)) / _2D::norm(l1.direction);
}

// Calculates the shortest distance between a point and a line
double _2D::distance_point_line(const pv& point, const lrs& line) {
    return fabs(_2D::cross(line.direction, point - line.start)) / _2D::norm(line.direction);
}

// Calculates the shortest distance between a point and a ray
double _2D::distance_point_ray(const pv& point, const lrs& ray) {
    if (_2D::dot(ray.direction, point - ray.start) < 0)
        return _2D::norm(point - ray.start); // Point is behind the start of the ray
...
bool _2D::point_inside_ray(const pv& point, const lrs& ray) {
    if (!_2D::point_inside_line(point, ray))
        return false;
...
bool _2D::point_inside_segment(const pv& point, const lrs& segment) {
    if (!_2D::point_inside_line(point, segment))
        return false;
    return _2D::dot(segment.finish - segment.start, point - segment.start) > -EPS &&
           _2D::dot(segment.start - segment.finish, point - segment.finish) > -EPS;
}
...
vector<pv> _2D::circle_line_intersection(const circle& _circle, const lrs& line) {
    pv projection = line.start + line.direction * (_2D::dot(_circle.C - line.start, line.direction) /
...
vector<pv> _2D::build_convex_hull(const vector<pv>& points) {
    ...
    for (int phase = 0; phase < 2; ++phase) {
        ...
        for (const pv& pt : P) {
            while (H.size() >= start + 2 && _2D::cross(H[H.size() - 1] - H[H.size() - 2], pt - H.back()) < EPS) {
                H.pop_back();
            }
            H.push_back(pt);
        }
...
int main() {
    ...
    cout << "Distance between point A and line CD: " << _2D::distance_point_line(A, lineCD) << endl;
    ...
    cout << "Polygon perimeter (ABCD): " << _2D::polygon_perimeter(rectangle) << endl;

    // Test point inside polygon
    pv P(2, 2); // Inside point
    cout << "Point P inside rectangle ABCD: " << (_2D::point_inside_polygon(P, rectangle) ? "Yes" : "No") << endl;

    // Test point on polygon boundary
    cout << "Point B on rectangle boundary: " << (_2D::point_on_polygon_boundary(B, rectangle) ? "Yes" : "No") << endl;

    // Test convex polygon check
    cout << "Rectangle ABCD is convex: " << (_2D::is_polygon_convex(rectangle) ? "Yes" : "No") << endl;

    // Test line intersection
    lrs lineAC(A, C), lineBD(B, D);
    auto intersection_points = _2D::line_line_intersect(lineAC, lineBD);
    if (!intersection_points.empty()) {
        cout << "Intersection of line AC and line BD: (" << intersection_points[0].x << ", " << intersection_points[0].y
             << ")" << endl;
    } else {
    ...
    cout << "Point P inside circle A: " << (_2D::point_inside_circle(P, circ) ? "Yes" : "No") << endl;
    ...
    if (!circle_line_intersections.empty()) {
        cout << "Intersection of circle A and line AB: (" << circle_line_intersections[0].x << ", "
    ...
    circle circ2(C, 3);
    auto circle_circle_intersections = _2D::circle_circle_intersection(circ, circ2);
    if (!circle_circle_intersections.empty()) {
        cout << "Intersection of circle A and circle C: (" << circle_circle_intersections[0].x << ", "
